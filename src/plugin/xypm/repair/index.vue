...
const options = reactive({
  api: api.getRepairList, // Adjust API method as needed
  rowSelection: { showCheckedAll: true },

  edit: {
    show: true,
    auth: ['/app/xypm/repair/edit'],
    func: async (record) => {
  ...
  delete: {
    show: true,
    auth: ['/app/xypm/repair/destroy'],
    func: async (params) => {
      console.log(params)
      const resp = await api.deleteRepair(params)
      if (resp.code === 200) {
...
const handleOk = async () => {
  ...
  const response = await api.handleRepair({
