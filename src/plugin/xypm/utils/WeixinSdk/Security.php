<?php
namespace plugin\xypm\utils\WeixinSdk;
...
class Security
{
	...
	{
		$http =sendRequest("https://api.weixin.qq.com/cgi-bin/token", [
		...
		if(!empty($json['access_token'])){
			$wxa = json_decode(sendRequest('https://api.weixin.qq.com/wxa/'.$security.'?access_token='.$json['access_token'], json_encode($data, JSON_UNESCAPED_UNICODE),'POST'), true);
