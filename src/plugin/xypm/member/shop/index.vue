...
import shopApi from "@/views/xypm/api/build/shop.js";
// 引用定义
const crudRef = ref()
const editRef = ref()
const mpRef = ref()
...
const options = reactive({
  ...
  add: {
    show: true,
    auth: ['/app/xypm/admin/member/save'],
    func: async () => {
      editRef.value?.open()
    },
  },
  import:{
    show: true,
    auth: ['/app/xypm/admin/member/import'],
    url: '/app/xypm/admin/member/import?type=shop',
    templateUrl: `${window.location.protocol}/xypm/file/商铺户主导入模板.xlsx`,
    func: async () => {
      editRef.value?.open()
    },
  },
  edit: {
    show: true,
    auth: ['/app/xypm/admin/member/edit'],
    func: async (record) => {
  ...
  delete: {
    show: true,
    auth: ['/app/xypm/admin/member/destroy'],
    func: async (params) => {
      console.log(params)
