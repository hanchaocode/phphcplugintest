...
class GarageController extends BaseController
{
    ...
    public function multiadd(Request $request) : Response
    {
        ...
        for($i = $startNum;$i <= $endNum;$i++){
            ...
            $garage = $this->logic->search(['number'=>$number])->find();
            if(empty($garage)){
                array_push($data,['number'=>$number]);
            }
        }
        ...
        if (!empty($ids)) {
            ...
            foreach ($list as $item) {
                if($item['status'] == 1){
                    ...
                    return $this->fail('储物间'.$item['number'].'已绑定户主，请先在户主管理-》储物间户主中删除对应户主信息');
                }
